...
// Muestra/oculta box_delta y ajusta su cota superior
...
static void on_size_value_changed(GtkSpinButton *spin, gpointer user_data) {
    AppWidgets *w = user_data;
    // Asegurar que n esté entre 3 y 12
    ...
    for (int i = 0; i < n; i++) {
        ...
        // Recálculo de Delta
        g_signal_connect(spin_ai, "value-changed", G_CALLBACK(on_ai_changed), w);
    ...
    // Se recorre spins[0..n-2] y se conecta con spins[i+1]
    ...
    // Si Delta ya está activo, se actualiza
...
static void on_execute_clicked(GtkButton *btn, gpointer data) {
    ...
    // 5) Se ejecuta una variante de backtracking
    if(gtk_toggle_button_get_active(
        GTK_TOGGLE_BUTTON(w->rb_v2))){
        delta = gtk_spin_button_get_value_as_int(w->spin_delta);
        sumaSubconjuntosV2_collect(A, n, W, 0, actual_idx, 0, 0, sol_list);
    ...
    {
        sumaSubconjuntosV4_collect(A, n, W, 0, actual_idx, 0, 0, sol_list);
        //g_print("[DEBUG] Variante MI Acotada escogida\n");
    }
    else
    {
        sumaSubconjuntosV1_collect(A, n, W, 0, actual_idx, 0, 0, sol_list);
    ...
    for (guint s = 0; s < sol_list->len; s++) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(chk), mask[i]);
...
int main(int argc, char *argv[]) {
    ...
    w->box_delta = GTK_BOX(gtk_builder_get_object(builder, "box_delta"));
    w->spin_delta = GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "spin_delta"));
    w->rb_delta = GTK_TOGGLE_BUTTON(gtk_builder_get_object(builder, "varianteDelta"));
    ...
    // Radios de variantes
    w->rb_v1 = GTK_TOGGLE_BUTTON(gtk_builder_get_object(builder, "varianteBasic"));
    w->rb_v2 = GTK_TOGGLE_BUTTON(gtk_builder_get_object(builder, "varianteDelta"));
    w->rb_v3 = GTK_TOGGLE_BUTTON(gtk_builder_get_object(builder, "varianteMayorIgual"));
    g_return_val_if_fail(GTK_IS_TOGGLE_BUTTON(w->rb_v3), 1);
    w->rb_v4 = GTK_TOGGLE_BUTTON(gtk_builder_get_object(builder, "varianteMIAcotado"));
    ...
    // Mostrar/ocultar Δ
    g_signal_connect(w->rb_delta, "toggled", G_CALLBACK(on_delta_toggled), w);
    ...
    // Oculta la caja de Δ si rb_delta no está activo
    on_delta_toggled(w->rb_delta, w);
